...
{
    public class DatabaseLookupExecutablesWithArguments : IActionSource2
    {
        ...
        {
            var m = Regex.Match(query, @"(?<exeName>\S+)\s+(?<arguments>.+)");

            if (! m.Success)
            {
                return Enumerable.Empty<IAction>();
            }
            var exeName = m.Groups["exeName"].Value;
            var arguments = m.Groups["arguments"].Value;
            query = exeName;

            using (new LogScope(log.Info, "exeName: {0}", query))
            {
